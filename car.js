class Car {
    constructor(
        ...
        if (controlType != "DUMMY") {
            ...
            this.brain = new NN(
                [this.sensor.rayCount, 6, 4], //[input, hidden, output] layer unit count
            );
        }
    ...
    #assessDamage(roadBorders, traffic) {
        for (let i = 0; i < roadBorders.length; i++) {
            if (detectIntersection(this.polygon, roadBorders[i])) {
                return true;
            }
        }

        for (let i = 0; i < traffic.length; i++) {
            if (detectIntersection(this.polygon, traffic[i].polygon)) {
                return true;
            }
        }
        return false;
